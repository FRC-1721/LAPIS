...
ticks_meter = 50000.0  # roughly calibrated on 8/25/2019
base_width = 0.75
enc_left = None
enc_right = None
...
while not rospy.is_shutdown():	# runs for as long as the node is running
    ...
    d = (d_left+d_right)/2
    dth = (d_right-d_left)/base_width
    dx = d / elapsed
    dr = th / elapsed

    if (d != 0):
        ddx = cos(th)*d  # fix these names
        ddy = -sin(th)*d
        x = x + (cos(th)*ddx - sin(th)*ddy)
        y = y + (sin(th)*ddx + cos(th)*ddy)
    if (dth != 0):
        th = th + dth
